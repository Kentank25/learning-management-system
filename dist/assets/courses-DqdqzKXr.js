import"./css-Cl0y6LQG.js";import{n as e,o as t}from"./buddy-vJ5S9m2q.js";(function(){let n=localStorage.getItem(`edu-level`)||`smk`,r=localStorage.getItem(`username`)||`Keane`,i=document.getElementById(`profile-name`),a=document.getElementById(`profile-img`);if(i&&(i.textContent=r),a){let e=`2563eb`;n===`sd`&&(e=`0ea5e9`),n===`smk`&&(e=`eab308`),n===`kuliah`&&(e=`6366f1`),a.src=`https://ui-avatars.com/api/?name=${encodeURIComponent(r)}&background=dbeafe&color=${e}`}let o=t[n],s=document.getElementById(`nav-dashboard-text`),c=document.getElementById(`nav-courses-text`),l=document.getElementById(`nav-calendar-text`),u=document.getElementById(`nav-grades-text`),d=document.getElementById(`nav-files-text`);s&&(s.textContent=o.dashboard),c&&(c.textContent=o.courses),l&&(l.textContent=o.calendar),u&&(u.textContent=o.grades),d&&(d.textContent=o.files);let f=document.getElementById(`courses-page-title`),p=document.getElementById(`courses-page-desc`);f&&(f.textContent=o.pageTitle),p&&(p.textContent=o.pageDesc);let m=e[n]||[],h=document.getElementById(`courses-grid`),g=e=>{if(h){if(h.innerHTML=``,e.length===0){h.innerHTML=`
      ...
      `,typeof lucide<`u`&&lucide.createIcons();return}e.forEach(e=>{let t=localStorage.getItem(`progress-course-${e.id}`),r=t===null?e.progress:parseInt(t),i=r===100,a=n===`sd`?`text-sky-600`:n===`kuliah`?`text-indigo-600`:`text-accent-600`,o=i?`bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100`:n===`sd`?`bg-sky-400 hover:bg-sky-500 text-white border-transparent`:n===`kuliah`?`bg-indigo-600 hover:bg-indigo-700 text-white border-transparent`:`bg-accent-400 hover:bg-accent-500 text-surface-900 border-transparent`,s=i?`Tinjau Materi (Selesai)`:r>0?`Lanjutkan Belajar`:`Mulai Belajar`,c=`
        <div
        ...
        >
          ...
          <div class="p-5 pt-8 flex-1 flex flex-col">
            <div class="flex-1">
              <h3 class="font-bold text-surface-900 text-lg mb-1 group-hover:${a} transition-colors line-clamp-2">${e.title}</h3>
            ...
            <div class="mt-auto">
              <div class="flex justify-between text-xs font-medium mb-1.5">
                ...
                <span class="${n===`sd`?`text-sky-500`:n===`kuliah`?`text-indigo-600`:`text-accent-600`} font-bold">${r}%</span>
              </div>
              <div class="w-full bg-surface-100 rounded-full h-1.5 mb-4">
                <div class="${n===`sd`?`bg-sky-500`:n===`kuliah`?`bg-indigo-600`:`bg-accent-500`} h-1.5 rounded-full" style="width: ${r}%"></div>
              </div>

              <a
                href="course-detail.html?id=${e.id}"
                class="block w-full py-2 text-center text-sm font-semibold border rounded-lg transition-all shadow-sm ${o}"
              >
                ${s}
              </a>
            </div>
